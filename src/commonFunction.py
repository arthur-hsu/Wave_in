...
class Common:
    ...
    def Line_Chart(self, excelPath, title, x_title, start_data_col, end_data_col, add_position):
        ...
        lineChat.style = 11
        lineChat.y_axis.title = 'Throughput'
...
def killProcess(keyword):
    ...
    ### Find the PID with name.
        ...
        while True:
            ...
            if tmp == '':
                break
            elif (len(items) > 3) and ('mount' not in items) and ('grep' not in items) and ('Studio' not in items) and ('/Applications/Vysor.app/Contents/Frameworks/Vysor' not in items) :
                print(tmp.replace('\n', ''))
